...
public class EnergyCableBlock extends AbstractEnergyBlock<Tier> implements IWaterLoggable, INoTileEnergy {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new EnergyCableTile(this.variant);
    }
    ...
    private BlockState createCableState(IWorld world, BlockPos pos) {
        ...
        return state.with(NORTH, north[0] && !north[1]).with(SOUTH, south[0] && !south[1]).with(WEST, west[0] && !west[1]).with(EAST, east[0] && !east[1]).with(UP, up[0] && !up[1]).with(DOWN, down[0] && !down[1]).with(TILE, tile).with(WATERLOGGED, ifluidstate.getFluid() == Fluids.WATER);
    }
    ...
    public boolean canConnectEnergy(IBlockReader world, BlockPos pos, Direction direction) {
        ...
        return !(tile instanceof EnergyCableTile) && Energy.isPresent(tile, direction);
    }
