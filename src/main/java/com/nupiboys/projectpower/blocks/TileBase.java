...
import com.nupiboys.projectpower.energy.Redstone;
import com.nupiboys.projectpower.energy.SideConfig;
import com.nupiboys.projectpower.inventory.Inventory;
import com.nupiboys.projectpower.util.Data;
import com.nupiboys.projectpower.util.Empty;
import com.nupiboys.projectpower.util.IVariant;
import com.nupiboys.projectpower.util.Safe;
import com.nupiboys.projectpower.util.Server;
import com.nupiboys.projectpower.util.Stack;
import com.nupiboys.projectpower.util.Text;

import javax.annotation.Nullable;

import static com.nupiboys.projectpower.blocks.AbstractBlock.LIT;
...
public class TileBase<E extends IVariant, B extends AbstractBlock<E>> extends TileEntity {
    ...
    public static class EnergyProvider<E extends IVariant, B extends AbstractEnergyProviderBlock<E>> extends EnergyStorage<E, B> {
        ...
        public void getListedEnergyInfo(List<String> list) {
            ...
            list.add(TextFormatting.GRAY + I18n.format("info.projectpower.max.generates", TextFormatting.DARK_GRAY + Text.numFormat(getGeneration())));
        }
    }
    ...
    public static class EnergyStorage<E extends IVariant, B extends AbstractEnergyBlock<E>> extends Tickable<E, B> implements IRedstoneInteract {
        ...
        public void getListedEnergyInfo(List<String> list) {
            ...
            list.add(g + I18n.format("info.projectpower.stored.energy.fe", dg + Text.addCommas(this.energy.getStored()), Text.numFormat(this.energy.getCapacity())));
            long ext = getMaxEnergyExtract();
            long re = getMaxEnergyReceive();
            if (ext + re > 0) {
                if (ext == re) {
                    list.add(g + I18n.format("info.projectpower.max.transfer.fe", TextFormatting.DARK_GRAY + Text.numFormat(ext)));
                } else {
                    if (ext > 0)
                        list.add(g + I18n.format("info.projectpower.max.extract.fe", dg + Text.numFormat(ext)));
                    if (re > 0)
                        list.add(g + I18n.format("info.projectpower.max.receive.fe", dg + Text.numFormat(re)));
                }
            }
        }
    }
